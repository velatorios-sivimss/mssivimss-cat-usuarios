...
public class RestTemplateUtil {
     ...
     * @return
     */
    public Response<?> sendPostRequestByteArray(String url, EnviarDatosRequest body, Class<?> clazz) throws IOException {
     ...
     * @return
     */
    public Response<?> sendPostRequestByteArrayToken(String url, EnviarDatosRequest body, String subject, Class<?> clazz) throws IOException {
    	Response<?> responseBody = new Response<>();
     ...
     * falta agregar el tema de seguridad para las peticiones
     *
     * @return
     */
     ...
     * @return
     */
    public Response<?> sendPostRequestByteArrayArchviosToken(String url, EnviarDatosArchivosRequest body, String subject, Class<?> clazz) throws IOException {
    	Response<?> responseBody = new Response<>();
    	...
    	for (MultipartFile file : body.getArchivos()) {
            if (!file.isEmpty()) {
            	parts.add("files", new MultipartInputStreamFileResource(file.getInputStream(), file.getOriginalFilename()));
            }
        }
    	
    	parts.add("datos", body.getDatos());
    	HttpEntity<LinkedMultiValueMap<String, Object>> request = new 
